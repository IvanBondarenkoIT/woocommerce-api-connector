...
### Решение 2: Установить User-Agent (обход без whitelist)

**Всегда устанавливайте User-Agent**, чтобы запросы выглядели как от браузера.

> **Подробное руководство:** [IMUNIFY360_BYPASS_WITHOUT_WHITELIST.md](IMUNIFY360_BYPASS_WITHOUT_WHITELIST.md) — технические детали, почему `session.headers` не работает, и чеклист.
