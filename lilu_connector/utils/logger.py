...
Логирование - это способ записывать информацию о работе программы.
Вместо print(), мы используем logger, который:
1. Записывает логи в файл
2. Показывает уровень важности (DEBUG, INFO, WARNING, ERROR)
3. Добавляет время и место, где произошло событие
...
def setup_logger(
    ...
    Для Junior: эта функция создаёт logger с двумя обработчиками:
    ...
    Пример:
        >>> # В начале модуля
        >>> from lilu_connector.utils.logger import setup_logger
    ...
    # Форматтер для логов
    # Для Junior: форматтер определяет, как будет выглядеть каждое сообщение
...
def get_logger(name: str) -> logging.Logger:
    ...
    Для Junior: удобная функция для получения logger без явной настройки.
